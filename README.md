...
#### 주제: Http 요청을 1만 건에서 50만 건 까지 증폭시키며 대규모 트래픽을 수용하는 프로젝트
- In Progress: 현재 100만 건의 요청에도 가용성은 확보되었으나,
CPU Usage 개선 여지가 있으며, Client 지표와 Server 지표가 다소 상이한 원인을 파악 중

![image.png](./imgs/ch04-http_scailing/grafana_view01.png)

## 🚀 성과 요약

### 1. [장애 극복] DB Deadlock 정밀 분석 및 장애율 0% 달성

#### 📌 배경:
- 동시 요청(1만)에서 `S-lock -> X-lock 상호 승격 시도`로 인한 **트랜잭션 교착상태(Deadlock)** 관측

#### 🔍 주요 내용:
- **InnoDB 강제 롤백**으로 인한 데이터 최종 정합성 소실 관측
- **DB Connection Full 점유**로 인한 Http 응답 지연 관측
- **Timeout**으로 인한 클라이언트 Http 요청 연쇄 실패 관측

#### ✅ 해결:
- `Insert Ignore`과 `Atomic Upsert Query`를 통한 **Lock 점유 최소화**

#### 📈 결과:
- Deadlock 해결과 **장애율 0%** 달성, 비관적/낙관적 락을 사용하지 않는 **도메인 친화적 해결책** 도입

---

### 2. [성능 개선] 캐시: Redis 전처리 계층 도입

#### 📌 배경:
- 동시 요청(10만)에서 **DB Connection 지연**으로 인한 요청 수용 실패

#### 🔍 주요 내용:
- 자체 **XOR 논리식**과 **Key Grouping**을 설계하여 데이터 압축 시도
- 일반적인 이벤트 저장 방식과 **Trade-off** 비교
- 배치 청크에 시간 역전 문제를 고려한 **Snowflake ID** 도입
- 설계에 대한 **논리적 타당성 검증**

#### ✅ 해결한 것:
- Like 요청 1번이 DB Connection과 `1:1 대응하는 구조`를 무너뜨림
- 원본 데이터(DB Table)를 거치지 않고도 **증분(delta)을 안전하게 집계**하는 구조 설계(Write-Back)
- DB Network I/O 집약적 구조를 **CPU I/O 위주**로 전환
- 분산 환경의 동시 요청 감지와 충돌 처리 구현

#### 📈 결과:
- 향후 Join Query에 유리한 구조로 개선, 병렬 배치의 기반이 되는 **chunk 설계**
...
#### 📌 배경:
- 순차적 배치만 가능하던 구조에서 **병렬 배치 구조**로 개선

#### 🔍 주요 내용:
- 논리 검증을 통한 **데이터 멱등성 보장** 확립
- `JdbcTemplate`를 활용한 병렬 배치 도입
- `MySQL Workbench`를 통한 **데이터 정합성 소실 여부** 관측
- **Redis Lua Script**를 통한 원자적 연산 수행

#### ✅ 해결한 것:
- **LLW 전략**과 **Optimistic Lock**을 혼합하여 배치 순서 역전 문제 해결
- **Staging Table**을 통한 Join 임시 테이블 구성으로 **Row Lock 지연** 개선
- **DB Ack Sign**을 통한 장애 복구 지점 설계

#### 📈 결과:
- 극단적인 환경제약(**Connection Pool=10, Timeout=1s**) 하에서도 100만 요청 가용성 확보 (**Fail=0, RPS=1200 내외 유지**)
- 단, Redis Hash 명령어에 개선 여지가 있어 **CPU 사용량 Spike 존재(약 30%)** 및 개선 중

---

### 4. [성능 개선] 클라이언트: 근본적인 부하 감소 시도

#### 📌 배경:
- 도메인에 적합한 정책을 세우고 **의미없는 요청을 정의하여 버리는 과정**

#### 🔍 주요 내용:
- 반복적인 `좋아요(Like)` 요청에서 **의미없는 요청**은 무엇인가
- 좋아요(Like)는 어느 정도의 **엄밀한 정합성**이 요구되는가
- 서버의 성능을 위해 어느 정도의 **UX 저하**를 감수할 수 있는가

#### ✅ 해결한 것:
- 응답을 받기 전에 UI를 렌더링하는 방식(**Optimistic UI**)으로 UX 개선
- 이에 따라 발생하는 서버-클라이언트 데이터 불일치 문제 대응

#### 📈 결과:
- 다수의 요청을 1번의 요청으로 통합하여 **서버 부하 감소**
- 단, 추가적으로 발생하는 문제는 문서(ch04-2)에서 제시하였음
...
- ch03, 04에 대해: 성과 중심으로 요악하기보다는, 편한 줄글 형식으로 개인적 인사이트를 담았습니다.
...
2. 클라이언트(Attacker CLI)의 Total Requst 지표와 서버(Grafana)지표가 상이
...
- HW Env: Intel i5-7300HQ(CPU), 16GB RAM
