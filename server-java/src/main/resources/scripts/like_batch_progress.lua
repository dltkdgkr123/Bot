...
-- [엄밀한 바운더리] 지수 표기법 방지를 위해 포맷팅하여 저장
...
-- [엄밀한 바운더리] 산술 연산 (resolved_time * 1048576)은 2^53을 넘어 정밀도가 파괴되므로 16진수 결합 사용
...
while snapshot_len < batch_size do
    local post_id = redis.call('SRANDMEMBER', KEYS[3])
    ...
    -- 해당 해시가 완전히 비었을 경우 명시적 삭제(레디스가 해주지만 방어 코드로써 작용) 및 대기열 제거
    if redis.call('HLEN', hash_key) == 0 then
        redis.call('SREM', KEYS[3], post_id)
